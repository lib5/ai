...
class DoubaoTester(BaseTester):
    """字节跳动豆包模型测试器"""

    def __init__(self, api_key: str, model: str = "doubao-seed-1-8-251228", **kwargs):
    ...
    def get_api_url(self) -> str:
        """返回豆包API端点"""
        return f"{self.base_url}/api/v3/chat/completions"
    ...
    async def chat_completion(self, messages: List[Dict]) -> Dict:
        ...
        payload = {
            ...
            "temperature": 0.7,
            "max_tokens": 4000,
            "reasoning_effort": "minimal"  # 不思考模式
        }
    ...
    def extract_content(self, response: Dict) -> str:
        ...
        try:
            if 'choices' in response and response['choices']:
                ...
                self.logger.warning(f"豆包响应格式异常: {response}")
                return str(response)
        except (KeyError, IndexError) as e:
            raise ValueError(f"无法从豆包响应中提取内容: {e}")
