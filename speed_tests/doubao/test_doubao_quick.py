...
async def run_single_test(tester: DoubaoTester, prompt: dict) -> dict:
    """运行单个测试"""
    prompt_type = prompt.get('type', 'text')
...
async def main():
    print("=" * 70)
    print("🚀 豆包模型快速测试 (1文本 + 1图片)")
    print("=" * 70)

    # 检查API密钥
    if not settings.doubao_api_key:
        print("❌ 错误: 未设置 DOUBAO_API_KEY")
        return 1
    ...
    output_file = f"doubao_quick_test_{timestamp}.json"
