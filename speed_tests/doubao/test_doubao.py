...
"""
豆包模型速度测试脚本
专门测试字节跳动豆包模型的响应速度和性能

使用方法:
1. 设置环境变量 DOUBAO_API_KEY
2. 运行测试: python test_doubao.py
"""
...
from speed_tests.doubao.adapter import DoubaoTester
...
def print_model_info(tester: DoubaoTester):
    """打印模型信息"""
    print("\n" + "=" * 60)
    print("🚀 豆包模型配置信息")
    print("=" * 60)
...
使用示例:
  ...
  # 使用自定义测试数据
  python test_doubao.py --prompts my_prompts.json

  # 设置请求间隔为3秒
  python test_doubao.py --delay 3

  # 使用指定的模型
  python test_doubao.py --model doubao-pro-4k
        """
    )
    ...
    try:
        ...
        output_file = f"doubao_speed_test_{timestamp}.json"
