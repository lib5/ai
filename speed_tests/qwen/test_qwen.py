...
# -*- coding: utf-8 -*-
"""
Qwen3-VL-Plus模型速度测试脚本
专门测试阿里云百炼Qwen3-VL-Plus的响应速度和性能

使用方法:
1. 设置环境变量 QWEN_API_KEY
2. 运行测试: python test_qwen.py
"""
...
from speed_tests.qwen.adapter import QwenTester
...
def print_model_info(tester: QwenTester):
    """打印模型信息"""
    print("\n" + "=" * 60)
    print("🚀 Qwen3-VL-Plus模型配置信息")
    print("=" * 60)
...
async def main():
    """主函数"""
    print("=" * 60)
    print("🚀 Qwen3-VL-Plus模型速度测试")
    print("=" * 60)

    # 解析命令行参数
    parser = argparse.ArgumentParser(
        description="Qwen3-VL-Plus模型速度测试工具",
...
使用示例:
  ...
  # 使用自定义测试数据
  python test_qwen.py --prompts my_prompts.json

  # 设置请求间隔为3秒
  python test_qwen.py --delay 3

  # 使用指定的模型
  python test_qwen.py --model qwen3-vl-plus
        """
    )
    ...
    parser.add_argument(
        '--model',
        type=str,
        default=settings.qianwen_model,
        help=f'Qwen模型名称 (默认: {settings.qianwen_model})'
    )

    parser.add_argument(
        '--timeout',
        type=int,
        default=settings.qianwen_timeout,
        help=f'请求超时时间，单位秒 (默认: {settings.qianwen_timeout})'
    )
    ...
    try:
        ...
        output_file = f"qwen_speed_test_{timestamp}.json"
