...
"""
147ai Gemini模型适配器
专门用于测试147ai的Gemini-3-Flash非流式输出速度
...
class One47aiGeminiTester(BaseTester):
    """147ai Gemini模型测试器"""
    ...
    def __init__(self, api_key: str, model: str = "gemini-3-flash-preview-low", **kwargs):
        ...
        super().__init__(
            model_name=f"147ai-Gemini-{model}",
            api_key=api_key,
            base_url=base_url,
            headers=headers,
            model=model,
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.post(
                ...
                if resp.status != 200:
                    ...
                    self.logger.error(f"147ai Gemini API错误 {resp.status}: {error_text}")
                    raise Exception(f"147ai Gemini API错误 {resp.status}: {error_text}")

                return await resp.json()

    def extract_content(self, response: Dict) -> str:
        """从147ai Gemini响应中提取内容"""
        try:
            if 'candidates' in response and response['candidates']:
                ...
                self.logger.warning(f"147ai Gemini响应格式异常: {response}")
                return str(response)
        except (KeyError, IndexError) as e:
            raise ValueError(f"无法从147ai Gemini响应中提取内容: {e}")
