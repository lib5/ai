...
"""
Gemini模型速度测试脚本
专门测试Google Gemini-3-Flash的响应速度和性能

使用方法:
1. 设置环境变量 OPENAI_API_KEY
2. 运行测试: python test_gemini.py
"""
...
def print_model_info(tester: GeminiTester):
    """打印模型信息"""
    print("\n" + "=" * 60)
    print("🚀 Gemini模型配置信息")
    print("=" * 60)
...
使用示例:
  ...
  # 使用自定义测试数据
  python test_gemini.py --prompts my_prompts.json

  # 设置请求间隔为3秒
  python test_gemini.py --delay 3

  # 使用指定的模型
  python test_gemini.py --model gemini-3-flash-preview
        """
    )
    ...
    try:
        ...
        output_file = f"gemini_speed_test_{timestamp}.json"
