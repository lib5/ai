...
"""
Gemini模型流式测试脚本
测试Gemini模型的TTFT (首Token时间) 性能

使用方法:
1. 设置环境变量 OPENAI_API_KEY
2. 运行测试: python test_gemini_stream.py
"""
...
def print_model_info(tester: GeminiStreamTester):
    """打印模型信息"""
    print("\n" + "=" * 60)
    print("🚀 Gemini模型流式测试配置")
    print("=" * 60)
...
async def main():
    """主函数"""
    print("=" * 60)
    print("🚀 Gemini模型流式测试 (TTFT)")
    print("=" * 60)

    # 解析命令行参数
    parser = argparse.ArgumentParser(
        description="Gemini模型TTFT测试工具",
...
使用示例:
  ...
  # 使用自定义测试数据
  python test_gemini_stream.py --prompts my_prompts.json

  # 设置请求间隔为3秒
  python test_gemini_stream.py --delay 3

  # 使用指定的模型
  python test_gemini_stream.py --model gemini-3-flash-preview
        """
    )
    ...
    parser.add_argument(
        ...
        help=f'Gemini模型名称 (默认: {settings.openai_model})'
    )
    ...
    try:
        ...
        output_file = f"gemini_stream_test_{timestamp}.json"
