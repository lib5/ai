...
"""
GPT-4模型流式测试脚本
测试GPT-4模型的TTFT (首Token时间) 性能

使用方法:
1. 设置环境变量 AZURE_OPENAI_API_KEY
2. 运行测试: python test_gpt4_stream.py
"""
...
def check_api_key():
    ...
    if not settings.azure_api_key:
        ...
        print("\n请设置环境变量:")
        print("   export AZURE_OPENAI_API_KEY='your_azure_openai_api_key'")
        print("\n或者在 .env 文件中添加:")
...
def print_model_info(tester: GPT4StreamTester):
    """打印模型信息"""
    print("\n" + "=" * 60)
    print("🚀 GPT-4模型流式测试配置")
    print("=" * 60)
...
async def main():
    """主函数"""
    print("=" * 60)
    print("🚀 GPT-4模型流式测试 (TTFT)")
    print("=" * 60)

    # 解析命令行参数
    parser = argparse.ArgumentParser(
        description="GPT-4模型TTFT测试工具",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
使用示例:
  # 基本使用
  python test_gpt4_stream.py

  # 使用自定义测试数据
  python test_gpt4_stream.py --prompts my_prompts.json

  # 设置请求间隔为3秒
  python test_gpt4_stream.py --delay 3

  # 使用指定的模型
  python test_gpt4_stream.py --model gpt-4.1
        """
    )
    ...
    try:
        ...
        output_file = f"gpt4_stream_test_{timestamp}.json"
