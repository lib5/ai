...
- 只要实际执行符合任一路径，即判定为正确
- 灵活的工具调用验证
- 🔧 失败重试机制：自动对验证失败的测试用例（status != "success"）进行重试验证（默认5次）
- 支持禁用重试机制(--no-retry)和自定义重试次数(--retry)
- 详细的重试统计和前后对比分析
"""
...
def fix_truncated_json(json_str):
    ...
    # 1. 检查是否缺少闭合引号（在字符串值中）
    # 简单检查：如果末尾的文本包含未闭合的引号
    last_quote_pos = json_str.rfind('"')
    if last_quote_pos > 0:
        # 检查从最后一个引号到结尾的文本
        after_last_quote = json_str[last_quote_pos+1:]
        # 如果末尾不是完整的字符串结构，尝试修复
        if after_last_quote and not after_last_quote.strip().startswith('",') and not after_last_quote.strip().startswith('"}'):
            # 尝试补全引号
    ...
    # 3. 检查是否缺少逗号（在对象或数组的最后一个属性后）
    # 如果最后一个字段后面缺少逗号，JSON.parse可能会失败
...
print("="*60)
print("测试结果验证工具 - 使用 Gemini-3-Flash-Preview")
print("="*60)
print()
    ...
    try:
        async with session.post(url, headers=headers, json=data, timeout=60) as response:
            ...
            try:
                ...
                logger.log(f"直接JSON解析失败: {str(e)}", "WARNING")
                # 尝试从文本中提取JSON
                import re
                # 使用贪婪匹配，避免被截断的情况
                ...
                if json_match:
                    ...
                    try:
                        ...
                        logger.log(f"从代码块提取JSON失败: {str(e2)}", "WARNING")
                        # 🔧 尝试修复被截断的JSON
                        logger.log("尝试修复被截断的JSON...", "INFO")
                        try:
                            # 尝试补全缺失的闭合括号
                            fixed_json = fix_truncated_json(json_content)
                            if fixed_json:
                                validation_data = json.loads(fixed_json)
                                logger.log("JSON修复成功！", "INFO")
                            else:
                                logger.log("JSON修复失败", "WARNING")
                        except Exception as e3:
                            logger.log(f"JSON修复过程出错: {str(e3)}", "WARNING")
                else:
                    logger.log("未找到JSON代码块", "WARNING")
...
async def main():
    ...
    parser.add_argument('--retry', '-r', type=int, default=10, help='失败重试次数 (默认: 5次)')
    ...
    # 用于跟踪需要重试的测试用例
    retry_cases = []
    all_results = []  # 存储所有验证结果
    initial_results = []  # 存储初始验证结果（用于对比）
    ...
    try:
        async with aiohttp.ClientSession() as session:
            for i, test_case in enumerate(test_cases_to_validate):
                ...
                initial_results.append(result)  # 保存初始结果

                # 🔄 重试机制：重试验证失败的用例（status != "success"）
                if result['status'] != 'success':
                    retry_cases.append((i, test_case))  # 保存索引和测试用例
                    logger.log(f"⚠️ 发现验证失败用例 (待重试): {result['test_case_id']} - status: {result['status']}, is_correct: {result['is_correct']}")

                # 记录所有业务逻辑错误的用例
                if result['is_correct'] == '错误':
                    problem_cases.append(result)

                if result['status'] == 'success' and result['is_correct'] != '错误':
                    logger.log(f"✅ 正常用例: {result['test_case_id']} - status: {result['status']}, is_correct: {result['is_correct']}")
            ...
            # 🔄 执行重试机制
            if enable_retry and retry_cases:
                logger.log("="*80)
                logger.log(f"开始重试机制，共 {len(retry_cases)} 个失败用例需要重试")
                print("="*60)
                print(f"开始重试机制，共 {len(retry_cases)} 个失败用例需要重试")
                print("="*60)
                ...
                while retry_cases and retry_attempt <= max_retries:
                    ...
                    print(f"\n第 {retry_attempt} 轮重试开始，共 {len(retry_cases)} 个用例")

                    current_retry_cases = retry_cases.copy()
                    retry_cases = []  # 清空，准备收集新一轮的失败用例

                    for case_index, (original_index, test_case) in enumerate(current_retry_cases):
                        logger.log(f"重试验证第 {case_index + 1}/{len(current_retry_cases)} 个: {test_case.get('test_case_id')}")
                        print(f"重试验证第 {case_index + 1}/{len(current_retry_cases)} 个: {test_case.get('test_case_id')}")

                        # 重新验证
                        result = await validate_single_case(session, test_case, original_index)
                        results[original_index] = result  # 更新结果
                        all_results[original_index] = result  # 更新所有结果

                        if result['status'] != 'success':
                            # 仍然失败，可能需要下一轮重试
                            if retry_attempt < max_retries:
                                retry_cases.append((original_index, test_case))
                                logger.log(f"❌ 第 {retry_attempt} 轮重试仍失败，将进入下一轮重试 - status: {result['status']}, is_correct: {result['is_correct']}")
                                print(f"❌ 第 {retry_attempt} 轮重试仍失败，将进入下一轮重试")
                            else:
                                logger.log(f"❌ 第 {retry_attempt} 轮重试失败，已达到最大重试次数 - status: {result['status']}, is_correct: {result['is_correct']}")
                                print(f"❌ 第 {retry_attempt} 轮重试失败，已达到最大重试次数")
                        else:
                            # 重试成功
                            logger.log(f"✅ 重试成功！用例 {test_case.get('test_case_id')} 现在验证通过 - status: {result['status']}, is_correct: {result['is_correct']}")
                            print(f"✅ 重试成功！用例 {test_case.get('test_case_id')} 现在验证通过")
                ...
                # 重试结束后，重新统计两个集合
                # 问题用例（业务逻辑错误）
                problem_cases = [r for r in results if r['is_correct'] == '错误']
                # 重试用例（验证过程失败）
                retry_cases = [r for r in results if r['status'] != 'success']

                logger.log(f"重试机制结束:")
                logger.log(f"  最终仍有 {len(problem_cases)} 个业务逻辑错误的用例")
                logger.log(f"  最终仍有 {len(retry_cases)} 个验证失败的用例")
                print(f"重试机制结束:")
                print(f"  最终仍有 {len(problem_cases)} 个业务逻辑错误的用例")
                print(f"  最终仍有 {len(retry_cases)} 个验证失败的用例")
    ...
    # 🔧 添加重试统计信息
    if enable_retry:
        logger.log(f"重试机制: 已启用, 最大重试次数: {max_retries}")
        print(f"重试机制: 已启用, 最大重试次数: {max_retries}")

        # 计算重试前后的对比
        initial_correct = sum(1 for r in initial_results if r['is_correct'] == '正确') if 'initial_results' in locals() else 0
        initial_wrong = sum(1 for r in initial_results if r['is_correct'] == '错误') if 'initial_results' in locals() else 0

        final_correct = sum(1 for r in results if r['is_correct'] == '正确')
        final_wrong = sum(1 for r in results if r['is_correct'] == '错误')

        recovered_cases = final_correct - initial_correct

        logger.log(f"重试前后对比:")
        logger.log(f"  重试前: 正确 {initial_correct}, 错误 {initial_wrong}")
        logger.log(f"  重试后: 正确 {final_correct}, 错误 {final_wrong}")
        logger.log(f"  通过重试修复: {recovered_cases} 个用例")
        print(f"重试前后对比:")
        print(f"  重试前: 正确 {initial_correct}, 错误 {initial_wrong}")
        print(f"  重试后: 正确 {final_correct}, 错误 {final_wrong}")
        print(f"  通过重试修复: {recovered_cases} 个用例")
    ...
    if successful > 0:
        ...
        # 🔧 如果仍有失败用例，给出提示
        if wrong > 0 and enable_retry and max_retries > 0:
            logger.log(f"⚠️ 注意: 已达到最大重试次数({max_retries})，仍有 {wrong} 个用例验证失败")
            print(f"⚠️ 注意: 已达到最大重试次数({max_retries})，仍有 {wrong} 个用例验证失败")
        elif wrong > 0:
            logger.log(f"⚠️ 注意: 重试机制已禁用，{wrong} 个用例验证失败")
            print(f"⚠️ 注意: 重试机制已禁用，{wrong} 个用例验证失败")
    ...
    if args.output:
        ...
        output_file = f"validation_reports/validation_report_{timestamp}.json"
    ...
    # 🔧 添加重试统计到报告
    if enable_retry:
        final_report['retry_summary'] = {
            'enabled': True,
            'max_retries': max_retries,
            'initial_correct': initial_correct if 'initial_correct' in locals() else 0,
            'initial_wrong': initial_wrong if 'initial_wrong' in locals() else 0,
            'final_correct': final_correct if 'final_correct' in locals() else 0,
            'final_wrong': final_wrong if 'final_wrong' in locals() else 0,
            'recovered_cases': recovered_cases if 'recovered_cases' in locals() else 0,
        }
    ...
    print(f"平均每个测试: {execution_time/len(results):.2f}秒")
    print("="*60)
...
4. 指定输入和输出文件：
   python validate_test_gemini_results.py -i test_results.json -o report.json

5. 限制验证数量（例如只验证前10个）：
   python validate_test_gemini_results.py --limit 10

示例场景：
- 如果初始验证有2个失败用例，系统会自动进行最多5次重试
- 重试过程中会显示每轮的进度和结果
- 最终报告会包含重试前后的详细对比
- 通过重试修复的用例数量会被统计和展示
""")
